(function() {
    ...
    function updateUI() {
        ...
        sendMessage({action: "getSetting", key: "username"}, function(response) {
            ...
            sendMessage({action: "getSetting", key: "abTests"}, function(response) {
                var ABTests = JSON.parse(response.value).tests;
                if(ABTests.chrome_extension_recs_v1.option === "control"){
                    $('#ril-recommendations').remove();
                }
            });
