...
(function (PKT_EXT) {
    ...
    var init = function init(options) {
        ...
        if (!recContainer) throw new Error("Invalid ID Parameters");

        recContainer.innerHTML = PKT_EXT.TEMPLATES.recommend({ loading: window.pkt.r_i18n['loading_recommendations'] });
    ...
    var buildRecs = function buildRecs(response, sourceID) {
        ...
        var listArray = Object.keys(response.data.list).map(function (key) {
            response.data.list[key].source_id = sourceID;
        ...
        response.data.list = listArray.slice(0, 3);
        response.data.more = response.data.reason ? window.pkt.r_i18n['more_on'] + ' ' : window.pkt.r_i18n['explore_recommendations'];
    ...
    var setNoRecs = function setNoRecs() {
        recList.innerHTML = PKT_EXT.TEMPLATES.recommendNoResults({
            ...
            explore_recs: window.pkt.r_i18n['explore_recommendations'] });
        setActions();
        showRecs();
    };
            ...
            if (recSave.hasClass('saved')) {
                sendMessage({
                    ...
                    if (response.status == 'success') {
                        ...
                        recSave.find('.pkt_ext_save_copy').text(window.pkt.r_i18n['save']);
                    }
                });
            } else {
                sendMessage(message, function (response) {
    ...
    var showRecs = function showRecs() {
        ...
        if (rightnow < self.loadThrottle) finalizeLoading();else setTimeout(finalizeLoading, loadThrottle - rightnow);
...
(function (PKT_EXT) {
    ...
    var init = function init(urlToSave, itemID) {
        ...
        saveContainer.innerHTML = PKT_EXT.TEMPLATES.save(window.pkt.r_i18n);
        ...
        optionPremium = window.___PKT__PREM_STATUS === 'true' || isLocal;
        optionSuggestions = window.___PKT__SUGGESTIONS === 'true' || isLocal;
        optionRecs = window.___PKT__RECS === 'true' || isLocal;

        PKT_EXT.TAGGING.init({
            containerName: 'pkt_ext_tag_input_wrapper',
            listContainerName: 'pkt_ext_suggested_tags',
            saveButtonName: '',
            placeholder: window.pkt.r_i18n['add_tags'],
            closePanel: closeSave
        });
    ...
    var showSave = function showSave(urlToSave, itemID) {
        ...
        PKT_EXT.TOOLBAR.setMessage(window.pkt.r_i18n['page_saved']);
    ...
    var saveURL = function saveURL(urlToSave, itemID) {

        if (!interfaceBuilt) init(urlToSave, itemID);else showSave(urlToSave, itemID);
    };
...
(function (PKT_EXT, taglib) {
    ...
    var suggestionListBuild = function suggestionListBuild(listItems) {
        ...
        if (!listItems || !listItems.length) {
            noTags.innerHTML = window.pkt.r_i18n['tags_suggested_empty'];
    ...
    var init = function init(options) {
        ...
        if (!container || !listContainer) throw new Error("Invalid ID Parameters");
    ...
    var initTagInput = function initTagInput(placeholder, container) {
        tagObject = new taglib(container, {
            ...
            onBeforeTagAdd: function onBeforeTagAdd(event, tag) {
                ...
                if (tag.length > 25) {
                    noTags.innerHTML = window.pkt.r_i18n['invalid_tags'];
...
(function (PKT_EXT) {
    ...
    var initRemovePageInput = function initRemovePageInput() {
        ...
        addAction(el, function (e) {
            ...
            sendMessage({
                ...
                if (response.status == 'success') {
                    setMessage(window.pkt.r_i18n['page_removed'], 'removed');
                    closePanel(true);
                }
            });
        });
    ...
    var initArchivePageInput = function initArchivePageInput() {
        $('.pkt_ext_action_archive').click(function (e) {
            ...
            sendMessage({
                ...
                if (response.status == 'success') {
                    setMessage(window.pkt.r_i18n['page_archived'], 'archived');
    ...
    var init = function init(options) {
        ...
        messagebox.text(window.pkt.r_i18n['page_saved']);
